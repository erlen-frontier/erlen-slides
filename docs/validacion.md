...
## Informes de Erlen DoE — 23 de septiembre de 2026 (0.3.0)

`tests/intercambio.test.mjs` comprueba con JSDOM y fake-indexeddb que el transporte es la copia literal de `web/exchange-v2.mjs`; que un `informe-v1` se valida y se convierte (portada, secciones partidas, tablas recortadas con su aviso, gráficas nativas con los mismos puntos, procedencia); que `#copy=<id>` se retira de la URL sin entrada en el historial, se previsualiza y solo al confirmar crea una presentación nueva en la biblioteca sin borrar la copia; que cancelar no crea nada y que un segundo envío no sobrescribe el primero; y que una copia de otro tipo, de otro destino, alterada, ausente o con un identificador inválido se rechaza con su motivo sin tocar la biblioteca ni la presentación abierta.

JSDOM no mide, así que el reparto por diapositiva se calibró y se comprobó en Chromium 141 (1400 × 900, tema Metropolis 16:9): cuerpo de 1280 × 601 px de lienzo; nueve bloques de texto de una línea, diez viñetas o una tabla de ocho filas cortas caben, y uno más no. Con esas medidas, dos informes de prueba —uno como el que arma DoE y otro hecho para desbordar— dieron 44 diapositivas de contenido sin nada fuera del cuerpo. Con otro tema o en 4:3 el reparto no se ha medido. La revisión «El contenido no cabe» de Preparar mi charla no detecta una tabla que desborda, porque el bloque de tabla se encoge dentro del cuerpo y recorta su contenido; la comprobación de arriba mide cada elemento.
